...
func (c *Controller) reconcileVirtualService(ctx context.Context, route *v1alpha1.Route, vs *v1alpha3.VirtualService) error {
	...
	vsClient := c.ElaClientSet.NetworkingV1alpha3().VirtualServices(vs.Namespace)
...
func (c *Controller) reconcileGateway(ctx context.Context, route *v1alpha1.Route) error {
	logger := logging.FromContext(ctx)
	gatewayNamespace := route.Namespace
	gatewayName := controller.GetElaK8SGatewayName(route)
	gateway := istio.MakeGateway(route)
	gatewayClient := c.ElaClientSet.NetworkingV1alpha3().Gateways(gatewayNamespace)
	existing, err := gatewayClient.Get(gatewayName, metav1.GetOptions{})
	if err != nil {
		if apierrs.IsNotFound(err) {
			if _, err = gatewayClient.Create(gateway); err == nil {
				logger.Infof("Created gateway %q in namespace %q", gatewayName, gatewayNamespace)
				c.Recorder.Eventf(route, corev1.EventTypeNormal, "Created", "Created Gateway %q in namespace %q", gatewayName, gatewayNamespace)
			}
		}
		return err
	}
	// Check if there is anything to update.
	if !reflect.DeepEqual(existing.Spec, gateway.Spec) {
		existing.Spec = gateway.Spec
		if _, err = gatewayClient.Update(existing); err == nil {
			logger.Infof("Updated gateway %q in namespace %q", gatewayName, gatewayNamespace)
			c.Recorder.Eventf(route, corev1.EventTypeNormal, "Updated", "Updated Gateway %q in namespace %q", gatewayName, gatewayNamespace)
		}
		return err
	}
	return nil
