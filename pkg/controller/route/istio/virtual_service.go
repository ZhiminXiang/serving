...
func makeVirtualServiceSpec(u *v1alpha1.Route, targets map[string][]traffic.RevisionTarget) v1alpha3.VirtualServiceSpec {
	...
	spec := v1alpha3.VirtualServiceSpec{
		...
		// the cluster.
		Gateways: []string{
			controller.GetElaK8SGatewayName(u),
			"mesh",
		},
		Hosts: []string{
			...
			controller.GetElaK8SServiceFullName(u),
		},
	}
	names := []string{}
...
func getRouteDomains(targetName string, u *v1alpha1.Route, domain string) []string {
	if targetName == "" {
		...
		return []string{domain, controller.GetElaK8SServiceFullName(u)}
	}
		...
		if t.Percent > 0 {
			weights = append(weights, v1alpha3.DestinationWeight{
				Destination: v1alpha3.Destination{
					Host: controller.GetServiceFullName(controller.GetElaK8SServiceNameForObj(t.TrafficTarget.RevisionName), ns),
...
func addActivatorRoutes(r v1alpha3.HTTPRoute, ns string, inactive []traffic.RevisionTarget) v1alpha3.HTTPRoute {
	...
	r.Route = append(r.Route, v1alpha3.DestinationWeight{
		Destination: v1alpha3.Destination{
			Host: fmt.Sprintf("%s.%s.svc.cluster.local", controller.GetElaK8SActivatorServiceName(), pkg.GetServingSystemNamespace()),
			Port: v1alpha3.PortSelector{
				Number: revision.ServicePort,
			},
		},
